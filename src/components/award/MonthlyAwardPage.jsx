// LeeeL's Award - 이 달의 유저 전용 페이지 (연도/월 선택)
const MonthlyAwardPage = ({ data = { matches: [], players: [] }, onGoHome, onGoAwardHub, onSearchPlayer }) => {
  ...
  // Extract available Year-Month options
  const availableMonths = useMemo(() => {
    ...
    matches.forEach(row => {
      ...
      if (d) {
        ...
        const label = `${y}년 ${m}월`;
        ymMap.set(key, { key, year: y, month: m, label });
      }
    });
    const sorted = Array.from(ymMap.values()).sort((a, b) => b.key.localeCompare(a.key));
    if (sorted.length > 0) return sorted;
    const now = new Date();
    return [{ key: `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2, "0")}`, year: now.getFullYear(), month: now.getMonth()+1, label: `${now.getFullYear()}년 ${now.getMonth()+1}월` }];
  }, [matches]);

  const [selectedYm, setSelectedYm] = useState(availableMonths[0].key);

  // Aggregate monthly stats
  const { participationTop10, winrateTop10, totalMatchesInMonth, currentLabel } = useMemo(() => {
    const statsMap = {};
    let count = 0;
    const targetObj = availableMonths.find(item => item.key === selectedYm) || availableMonths[0];

    matches.forEach(row => {
      const d = parseGameDate(row[21]);
      if (!d) return;
      if (d.getFullYear() !== targetObj.year || (d.getMonth() + 1) !== targetObj.month) return;
  ...
  }, [matches, selectedYm, availableMonths]);
    ...
    <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-slate-400">
            {currentLabel} 한 달간 가장 뜨거운 활약을 펼친 시참왕과 승률왕입니다.
          </p>
        </div>
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-2xl p-4 sm:p-5 shadow-xl flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {availableMonths.map(item => (
            <button
              key={item.key}
              onClick={() => setSelectedYm(item.key)}
              className={`px-4 py-2 rounded-xl text-xs sm:text-sm font-bold transition-all ${
                selectedYm === item.key
                  ? "bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-lg shadow-indigo-950/50 scale-105"
        ...
        <div className="text-xs text-slate-400 font-medium">
          총 <strong className="text-indigo-400">{totalMatchesInMonth}경기</strong> 분석됨
        </div>
      </section>
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-black text-white flex items-center space-x-2">
                <span>⚡ {currentLabel} 이달의 시참왕</span>
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">이 달에 가장 많은 경기를 함께한 열정 랭커 TOP 10</p>
            </div>
          </div>
        ...
        {participationTop10.length > 0 ? (
          ...
          <div className="py-12 text-center text-slate-500 text-sm">해당 월의 경기 데이터가 없습니다.</div>
        )}
      </section>
      ...
      <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl space-y-6">
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-xl font-black text-white flex items-center space-x-2">
                <span>🎯 {currentLabel} 이달의 승률왕</span>
              </h3>
        ...
        {winrateTop10.length > 0 ? (
          ...
          <div className="py-12 text-center text-slate-500 text-sm">해당 월에 최소 5판 이상 플레이한 유저가 없습니다.</div>
        )}
      </section>
